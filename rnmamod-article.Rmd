...
`metareg_plot()` calls `scatterplot_sucra()` and `forestplot_sucra()` to produce the intended graphs and displays tables in the console, comparing the effect estimates and predictions from NMA with those from network meta-regression. Every type of analysis is equipeed with a unique visualisation toolkit, represented by the functions sharing the same colour node (Figure \@ref(fig:network-visualisation)). Only network meta-regression (blue nodes) and separate pairwise meta-analyses (green nodes) share a few stand-alone functions with NMA (grey nodes),
