...
their outcome can only be hypothesised with some uncertainty.
Hence, we can determine a distribution of possible values to describe the hypothetical outcomes of missing
...
the number randomised participants, the number of completers and missing participants, and the 
...
parameter $\theta^{m}_{ik}$ as a function of an informative missingness parameter $\phi_{ik}$, a difference
of the outcome in completers and missing participants [@White2008; @Mavridis2015; @Turner2015]:
...
The response probability ratio [@Magder2003; @Turner2015] aligns better with a different approach for dealing with missing data, i.e., the   selection model. This model distinguishes the participants based on their outcome and then further categorises them into those who completed the assigned 
...
the mean difference (MD) and standardised mean difference, respectively.
...
assumption (ignorable missingness), while non-zero values indicate the missing-not-at-random assumption  (non-ignorable missingness). Essentially, these informative 
missingness parameters quantify deviations from the missing-at-random assumption. 
As these parameters are typically unknown, the analysts can consider one of the following 
situations:
...
and our uncertainty about this belief, respectively [@Higgins2008; @White2008; 
@Mavridis2015; @Turner2015; @Spineli2019; @Spineli2021]:
...
[@Spineli2019; @SpineliStatMed2019]. The authors provide recommendations to assist 
...
review [@Spineli2019; @SpineliStatMed2019]. They authors also highlight that further 
research is needed in demystifying further the properties of these structural assumptions 
[@Spineli2019; @SpineliStatMed2019]. The __rnmamod__ package allows the user to 
apply all structural specifications for log IMOR, IMDoM and log IMRoM. 

In this article, we will only consider IMOR and IMDoM due to their intuitive 
relationship to the aforementioned effect measures, which are also the most frequently 
employed in published systematic reviews and relevant methodological literature 
...
Initially, `run_model()` calls  the `data_preparation()` function to appropriately prepare the format of the dataset for JAGS modelling. 
The dataset is provided in the one-study-per-row format, which is
...
NMA, following the methodology published by the NICE Decision Support Unit in a series of tutorial papers  on evidence synthesis methods 
for decision-making [@Dias2013]. The `missingness_param_prior()` and `heterogeneity_param_prior()` functions process the hyperparameters of the 
selected prior distribution for the informative missingness parameter and the  between-study heterogeneity parameter, respectively, preparing them for use by JAGS. 
