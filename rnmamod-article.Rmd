...
community [@GetRealNMA; @Petropoulou2017]. The availability of specialised statistical 
software has been the driving force behind the progress and widespread adoption of NMA. 
A comprehensive review of the methodology and software for NMA [@GetRealNMA] listed 
several statistical software tools that have been instrumental in promoting NMA. 
Among these tools, the __R__ software [@R2023] being the most popular, followed 
by __Stata__ [@Stata] and __SAS software__ [@SAS2020]. In the past decade, there 
has been a rise in the development of R packages tailored for NMA, each offering 
specific functionalities [@CRANTaskReview].
...
package __rnmamod__ [@rnmamod], published recently in the Comprehensive R Archive 
...
outcome data in NMA. Section [3](#The-architecture-of-\pkg{rnmamod}) delineates 
the architecture of __rnmamod__, and section [4](#X) exemplifies the several 
...
the informative missingness odds ratio in the logarithmic scale for binary 
...
the informative missingness difference of means for continuous outcomes [@Mavridis2015]. 
Other informative missingness parameters that have been suggested for binary outcomes 
are the response probability ratio [@Magder2003; @Turner2015] defined as the ratio 
of the probability of completing the trial given the outcome being experienced to 
the probability of completing the trial given the outcome not being experienced,

$$\begin{aligned}
\omega_{ik} = \dfrac{P(M_{ikj} = 0 | I_{ikj} = 1)}{P(M_{ikj} = 0 | I_{ikj} = 0)}
\end{aligned}$$

and the success probability ratio [@Akl2013; @Turner2015] as the ratio of the probability 
of experiencing the outcome given the missing participants to the probability of 
experiencing the outcome given the completers,

$$\begin{aligned}
\rho_{ik} = \dfrac{\theta^{m}_{ik}}{\theta^{c}_{ik}} = \dfrac{P(I_{ikj} = 1 | M_{ikj} = 1)}{P(I_{ikj} = 1 | M_{ikj} = 0)}.
\end{aligned}$$

Finally, the informative missingness ratio of means has been also for the continuous 
outcomes [@Mavridis2015] defined as the mean outcome given the missing participants 
to the mean outcome given the completers,

$$\begin{aligned}
\zeta_{ik} = \dfrac{\theta^{m}_{ik}}{\theta^{c}_{ik}} = \dfrac{E(Y_{ikj} | M_{ikj} = 1)}{E(Y_{ikj} | M_{ikj} = 0)}.
\end{aligned}$$

However, IMOR and IMDoM have been preferred in the literature for conceptually 
aligning with the odds ratio and the mean difference and standardised mean difference, 
respectively, which are also the most frequently used effect measures in published 
systematic reviews and relevant methodology [@Friedrich2008; ]. Both
